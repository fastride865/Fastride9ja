...
class ManualDispatchController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('taxicompany.manual.index', compact('config', 'paymentmethods', 'corporates', 'countries', 'baseConfig'));
    }
    ...
    public function TestIndex(){
        ...
        $countries = Country::where([['merchant_id', '=', $merchant_id], ['country_status', '=', 1]])->get();
        $paymentmethods = PaymentMethod::get();
        return view('taxicompany.manual.manual', compact('taxicompany','config', 'paymentmethods', 'corporates', 'countries', 'baseConfig','bookingConfig'));
    }
...
//        $this->SetTimeZone($request->manual_area);
        ...
        switch ($request->driver_request) {
            case "1":
                ...
                if (empty($drivers) && \request()->booking_type == 1) {
                    return redirect()->back()->withErrors(trans('api.no_driver_found'));
                }
            ...
            case "3":
                ...
                if (empty($drivers)) {
                    return redirect()->back()->withErrors(trans('admin.no_driver_found'));
                }
        ...
        return redirect()->route('taxicompany.ride-requests', $booking->id)->with('success');
    }
...
//            return array('price_card_id' => $price->id,'amount' => $amount);
//        }
//    }
...
//        $this->SetTimeZone(request()->manual_area);
    ...
    public function SearchUser(Request $request)
    {
        ...
        $rider = User::where([['merchant_id', '=', $merchant_id], ['UserPhone', '=', $request->user_phone],['taxi_company_id', '=', $taxi_company->id],['user_delete','=', NULL]])->first();
        $id = NULL;
        $gender = '';
        if(!empty($rider->id))
        {
