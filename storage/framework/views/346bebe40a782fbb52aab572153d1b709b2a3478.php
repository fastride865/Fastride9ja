...
<footer class="site-footer">
  <div class="site-footer-legal">© <?php echo e(date('Y').' '.$merchant->BusinessName); ?></div>
...
<script src="<?php echo e(asset('theme/examples/js/forms/editor-summernote.js')); ?>"></script>
...
<script src="<?php echo e(asset('global/vendor/clockpicker/bootstrap-clockpicker.js')); ?>"></script>
...
<script src="<?php echo e(asset('global/vendor/timepicker/jquery.timepicker.min.js')); ?>"></script>
...
<script>
  ...
  <?php if(isset($merchant->OneSignal->web_application_key) && !is_null($merchant->OneSignal->web_application_key)): ?>
  var OneSignal = window.OneSignal || [];

  OneSignal.push(function () {
    OneSignal.init({
      appId: "<?php echo e($merchant->OneSignal->web_application_key); ?>",
      welcomeNotification: {
        title: "<?php echo e($merchant->BusinessName); ?>",
      },
      notifyButton: {
        enable: true,
      ...
      if (typeof (event.data.type) != "undefined" && event.data.type !== null) // type = 1 : NEW BOOKING
      {
        ...
        switch (event.data.type) {
          case 1:
            ...
            swal({
              title: "<?php echo app('translator')->get('admin.newbooking'); ?>",
              text: event.content,
              icon: "success",
              buttons: ["<?php echo app('translator')->get("$string_file.no"); ?>", "<?php echo app('translator')->get("$string_file.yes"); ?>"],
            }).then((isConfirm) => {
              if (isConfirm) {
                location.href = event.url;
              } else {
                swal("<?php echo app('translator')->get("$string_file.thanks"); ?>");
              }
            });
            <?php endif; ?>
            break;
          case 2:
            swal({
              title: "<?php echo app('translator')->get('admin.no_driver_found'); ?>",
              text: event.content,
              icon: "success",
              buttons: ["<?php echo app('translator')->get("$string_file.no"); ?>", "<?php echo app('translator')->get("$string_file.yes"); ?>"],
            }).then((isConfirm) => {
        ...
        case 3:
            swal({
              title: "<?php echo app('translator')->get('admin.new_driver_register'); ?>",
              text: event.content,
              icon: "success",
              buttons: ["<?php echo app('translator')->get("$string_file.no"); ?>", "<?php echo app('translator')->get("$string_file.yes"); ?>"],
            }).then((isConfirm) => {
...
</html><?php /**PATH /home/msprojectsappori/public_html/multi-service-v1/resources/views/merchant/layouts/footer.blade.php ENDPATH**/ ?>